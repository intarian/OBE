...
<?php //What page to show
    ...
    if(mysqli_fetch_assoc(mysqli_query($conn,"SELECT course_ended FROM courses_info WHERE courses_info.`course_id` ='$course'"))['course_ended']){
        header('Refresh: 0; URL=main.php?error=The course is marked ended, write access is not permitted&menu=atten_display&module='.$module.'&course='.$course.'');
        exit();
    }
...
<div class="w3-container">
    <h2 align="center"><b>Attendance Marker</b></h2>
    <br>
    <?php if($method=='mark'){
    $sess = ((int)($roll/1000));
    $roll = $roll-($sess*1000);    
    ...
    $sql_run = mysqli_query($conn, "UPDATE `$course` SET `$module` = 'P' WHERE data_detail = '$roll_orig'");
    if(mysqli_affected_rows($conn))
       echo "<div align=\"center\"><strong style=\"color:#008000;\">Attendance Marked for ".$roll_orig."&nbsp;&nbsp;(".$name.")</strong><div>";
    else 
       echo "<div align=\"center\"><strong style=\"color:#FF3333;\">Attendance already marked or Roll not exist in the current roster list</p></strong><div>";
   }
    ?>
    <form id="attendance" method="get" action="">
    ...
    <tr><td><b>Date: </b></td>
        <td><b> <?php echo date_format(date_create($date),"d/m/Y") ?></b></td>
        </tr>
    ...
    <div align="center" style="margin-top: 20px;">
        <input class="sub" type="submit" value="Mark Attendance" />
        <br><br><br>
    <a href="main.php?menu=atten_display&course=<?php echo $course ?>&module=<?php echo $module ?>">See Marked Attendance or edit Manually</a>
        </div>
 </form> 
    </div>
<body>
